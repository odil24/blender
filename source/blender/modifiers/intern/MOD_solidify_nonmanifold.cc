            ...
            for (uint k = 0; k < edges_num && can_merge; k++) {
                  ...
                  (ELEM(vm[orig_edges[k][0]], v1, v2) != ELEM(vm[orig_edges[k][1]], v1, v2)))
              {
                for (uint j = 0; j < edge_adj_faces[k]->faces_len && can_merge; j++) {
                   ...
                   * Without this check a duplicate edge would be created, see: #150854.
                   *
                   * Logically this could be done with a `set` referencing unique vertices
                   * after the merge was done, however it is simpler to track that there is
                   * at least two other unique vertices.
                   *
                   ...
                   * Tracking this topology gets a bit involved but it's possible.
